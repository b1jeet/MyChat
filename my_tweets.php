...
if (mysqli_num_rows($r) > 0) {
    ...
    while($row = mysqli_fetch_assoc($r)) {
         ...
         function displaytweets()
      {
        ...
        global $con;
        global $h;
        $a_query="SELECT * FROM tweets WHERE userid= ".$h;
        $a_result=mysqli_query($con,$a_query);
       while($row=mysqli_fetch_assoc($a_result))
        {
            $userquery="SELECT * FROM login WHERE id=".mysqli_real_escape_string($con,$h);
            ...
            echo "<p>".$row['tweet']."</p>";
            echo "<form action='unfollow.php' method='POST'><p><input type='hidden' id='userid' name='userId' value='".$row['userid']."'></p><p><input type='hidden' id='id' name='id' value='".$h."'></p></form></div>";
        }
...
displaytweets();
   ...
   function displayTweetBox() {
            ...
            echo '<form class="form-inline" action="posttweet.php" method="post">
                ...
                &nbsp
                <button type="submit" class="btn btn-danger">Post Tweet</button>
                </div>
            </form>';
