...
$userquery="SELECT * FROM login WHERE id=".mysqli_real_escape_string($con,$j);
            $userqueryresult=mysqli_query($con,$userquery);
            $user = mysqli_fetch_assoc($userqueryresult);
        echo "<p><h2>".$user['email']."'s Tweets:</h2></p>";    
 $a_query="SELECT * FROM tweets WHERE userid= ".$j;
        $a_result=mysqli_query($con,$a_query);
       while($row=mysqli_fetch_assoc($a_result))
        {
            echo "<div class='tweet'><p>".$row['tweet']."</p></div>";
        }
